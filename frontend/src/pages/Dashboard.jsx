...
function MiniSalesChart({ data, labels }) {
  const width = 640
  const height = 220
  const padding = { top: 16, right: 18, bottom: 30, left: 10 }
  const w = width - padding.left - padding.right
  const h = height - padding.top - padding.bottom

  const max = Math.max(...data)
  const points = data.map((v, i) => {
    const x = padding.left + (w * i) / (data.length - 1)
    const y = padding.top + (h * (1 - v / max))
    return { x, y, v }
  })
  ...
  return (
    <div className="h-72 w-full">
      <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full" role="img" aria-label="Graphique ventes 7 jours">
        <defs>
        ...
        {Array.from({ length: 5 }).map((_, idx) => {
          const y = padding.top + (h * idx) / 4
          return <line key={idx} x1={padding.left} x2={padding.left + w} y1={y} y2={y} stroke="#E2E8F0" strokeDasharray="4 6" />
        })}

        {/* x labels */}
        {labels.map((lab, i) => {
          const x = padding.left + (w * i) / (labels.length - 1)
          return (
            <text key={lab} x={x} y={height - 12} fill="#64748b" fontSize="12" textAnchor="middle">
              {lab}
            </text>
          )
        })}
        ...
        {/* line */}
        <path d={pathD} fill="none" stroke="url(#salesStroke)" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />

        {/* points */}
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="5" fill="#EEF2FF" stroke="#6366F1" strokeWidth="2" />
          </g>
        ))}
      </svg>
    </div>
...
export default function Dashboard() {
  ...
  const filteredSoonExpiring = useMemo(() => {
    ...
    return soonExpiring.filter((m) => m.name.toLowerCase().includes(q) || m.batch.toLowerCase().includes(q))
  }, [query, soonExpiring])
  ...
  return (
    <div className="min-h-screen bg-slate-50">
            ...
            <div className="flex items-center justify-between px-6 py-5">
              <div className="flex items-center gap-3">
                ...
                <div className="leading-tight">
                  <div className="text-[15px] font-extrabold tracking-tight text-slate-900">PharmaPro</div>
            ...
            <nav className="flex-1 overflow-y-auto px-2 pb-5">
              <div className="space-y-1">
                {menuItems.map((item, idx) => {
                  ...
                  const route = getRouteForKey(item.key)
                  const content = (
                    <>
                      <span
                        className={cn(
                      ...
                      >
                        <Icon className={cn('h-5 w-5 transition-all', isActive ? 'text-white' : 'text-indigo-600')} />
                      </span>
                      ...
                      {isActive ? (
                        ...
                        <span className="ml-auto opacity-0 transition-opacity group-hover:opacity-100 text-slate-400">→</span>
                      )}
                    </>
                  )
          ...
          <div className="px-6 py-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className={cn('grid h-12 w-12 place-items-center rounded-2xl', s.iconBg)}>
                    ...
                    <Link
                      to={s.title === 'Nombre total de médicaments' || s.title === 'Stock faible' ? '/stock' : '/add'}
                      className="mt-4 inline-flex items-center gap-2 rounded-full bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600 transition hover:bg-white hover:shadow"
                    >
                      Détails
            ...
            <section className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3">
              ...
              <div className="xl:col-span-2 rounded-2xl bg-white p-5 shadow-md ring-1 ring-slate-100/60">
                ...
                <MiniSalesChart data={sales7Days.values} labels={sales7Days.labels} />
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-xs font-semibold text-slate-500">Objectif : +10%</span>
                </div>
              </div>
            ...
            <section className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
              <div className="xl:col-span-1 rounded-2xl bg-white p-5 shadow-md ring-1 ring-slate-100/60">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="inline-flex items-center rounded-full bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700">Top 5</span>
                </div>
              ...
              <div className="xl:col-span-2 rounded-2xl bg-white p-5 shadow-md ring-1 ring-slate-100/60">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="inline-flex items-center rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600">Live</span>
                </div>
      ...
      {/* Responsive sidebar spacing (simple) */}
      <style>{`
