...
export default function Stock() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // API endpoint Django
        // Backend base URL for API requests (fix for “Failed to fetch”).
        // Django API endpoint lives at /products/api/medicaments/.
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `HTTP ${res.status}. endpoint: ${endpoint}. Body: ${text.slice(0, 120)}`
          );
        }
        ...
        if (!cancelled) {
          setError(`Impossible de charger le stock. ${e?.message ?? ""}`);
        }
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((m) => {
      ...
      const matchesLow =
        !onlyLowStock || Number(m.quantite_stock ?? 0) < Number(minQty ?? 10);

      return matchesQuery && matchesLow;
    });
  }, [items, query, onlyLowStock, minQty]);
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Nombre total de médicaments : <span className="font-bold">{totalCount}</span>
          </p>
        </div>
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="text-sm font-semibold text-slate-700">
          Résultats : <span className="text-indigo-700">{filtered.length}</span>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-6 text-slate-600">Aucun résultat.</div>
        ) : (
                  ...
                  >
                    ...
                    <td
                      className={`p-3 text-sm font-semibold ${
                        Number(m.quantite_stock ?? 0) < Number(minQty ?? 10)
                          ? "text-rose-700"
                          : "text-emerald-700"
                      }`}
                    >
